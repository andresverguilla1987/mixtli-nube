...
app.get("/salud",(req,res)=>res.json({ ok:true, relay:true, time:new Date().toISOString() }));

// Accepts multipart/form-data field "file"
app.post("/api/upload", upload.single("file"), async (req, res) => {
  try {
    const client = s3();
    if (!client) return res.status(500).json({ ok:false, error:"S3 not configured" });
...
app.listen(PORT, ()=>console.log("Mixtli Transfer Relay on", PORT));
